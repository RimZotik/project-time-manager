...
Project Time Manager — настольный тайм-трекер для Windows. Приложение помогает вести учет времени по проектам: вы выбираете проект, запускаете запись, а программа считает, какие приложения и сайты в браузере были активны во время работы.
...
- Разворачивать браузеры до сайтов и хранить список посещенных ссылок по каждому сайту.
- Включать и исключать приложения или сайты из итогового времени без удаления реальных данных.
- Создавать этапы проекта и выбирать один или несколько этапов перед стартом сессии.
...
2. Нажмите `Старт`.
3. При необходимости выберите этапы проекта: они зафиксируются на всю сессию до `Стоп`.
4. Работайте как обычно: монтаж, разработка, браузер, документы.
5. При необходимости отключайте галочки у приложений или сайтов, которые не должны попадать в итог.
6. Нажмите `Пауза` или `Стоп`.
7. Экспортируйте Excel или PDF отчет.

Пока сессия не остановлена полностью, импорт, экспорт и изменение этапов отключены, чтобы отчет не собирался из полузаписанных данных.
